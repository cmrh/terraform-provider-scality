...
func testAccBucketACLBase(name string) string {
...
func TestAccBucketACL_basic(t *testing.T) {
	name := acctest.RandomName("acctest")

	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { acctest.PreCheckConsole(t) },
		ProtoV6ProviderFactories: acctest.TestProtoV6ProviderFactories,
		CheckDestroy:            acctest.CheckResourceDestroyed("scality_bucket_acl"),
		Steps: []resource.TestStep{
			{
				Config: testAccBucketACLBase(name) + `
resource "scality_bucket_acl" "test" {
  account_access_key = scality_user_access_key.test.access_key_id
  account_secret_key = scality_user_access_key.test.secret_access_key
...
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("scality_bucket_acl.test", "bucket", name+"-bucket"),
					resource.TestCheckResourceAttr("scality_bucket_acl.test", "acl", "public-read"),
				),
			},
		},
	})
