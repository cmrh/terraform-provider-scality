...
// ---------------------------------------------------------------------------
// WaitForCustomAttributes — polls GetAccount until customAttributes converge.
// Tests shorten the timeout/backoff via the package-level vars so they don't
// burn real time.
// ---------------------------------------------------------------------------

// withFastPolling shrinks WaitForCustomAttributes' clocks for tests.
func withFastPolling(t *testing.T, timeout, initial, maxBackoff time.Duration) {
	t.Helper()
	origTimeout := customAttrsPollTimeout
	origInitial := customAttrsInitialBackoff
	origMax := customAttrsMaxBackoff
	customAttrsPollTimeout = timeout
	customAttrsInitialBackoff = initial
	customAttrsMaxBackoff = maxBackoff
	t.Cleanup(func() {
		customAttrsPollTimeout = origTimeout
		customAttrsInitialBackoff = origInitial
		customAttrsMaxBackoff = origMax
	})
}

func getAccountServer(t *testing.T, responses ...string) *httptest.Server {
	t.Helper()
...
func TestWaitForCustomAttributes_ConsistentImmediately(t *testing.T) {
	withFastPolling(t, 1*time.Second, 10*time.Millisecond, 50*time.Millisecond)
	server := getAccountServer(t, `{
		"arn": "arn:aws:iam::123:root", "canonicalId": "abc", "id": "123",
		"emailAddress": "e@e", "name": "test", "createDate": "2026-01-01",
		"quotaMax": 0,
		"customAttributes": {"env": "prod"}
	}`)
	defer server.Close()

	client := NewIAMClient(server.URL, "ak", "sk", false)
	err := client.WaitForCustomAttributes(context.Background(), "test", map[string]string{"env": "prod"})
	if err != nil {
		t.Fatalf("WaitForCustomAttributes returned error: %v", err)
	}
}

func TestWaitForCustomAttributes_ConvergesAfterPolls(t *testing.T) {
	withFastPolling(t, 1*time.Second, 5*time.Millisecond, 20*time.Millisecond)
	stale := `{
		"arn": "arn:aws:iam::123:root", "canonicalId": "abc", "id": "123",
		"emailAddress": "e@e", "name": "test", "createDate": "2026-01-01",
		"quotaMax": 0,
		"customAttributes": {"env": "old"}
	}`
	fresh := `{
		"arn": "arn:aws:iam::123:root", "canonicalId": "abc", "id": "123",
		"emailAddress": "e@e", "name": "test", "createDate": "2026-01-01",
		"quotaMax": 0,
		"customAttributes": {"env": "new"}
	}`
	server := getAccountServer(t, stale, stale, fresh)
	defer server.Close()

	client := NewIAMClient(server.URL, "ak", "sk", false)
	err := client.WaitForCustomAttributes(context.Background(), "test", map[string]string{"env": "new"})
	if err != nil {
		t.Fatalf("WaitForCustomAttributes returned error: %v", err)
	}
}

func TestWaitForCustomAttributes_ConvergesFromEmpty(t *testing.T) {
	// Covers the #68 case: GetAccount returns no customAttributes immediately
	// after a Create+UpdateAccountAttributes, then settles.
	withFastPolling(t, 1*time.Second, 5*time.Millisecond, 20*time.Millisecond)
	empty := `{
		"arn": "arn:aws:iam::123:root", "canonicalId": "abc", "id": "123",
		"emailAddress": "e@e", "name": "test", "createDate": "2026-01-01",
	...
	server := getAccountServer(t, empty, fresh)
	defer server.Close()
...
func TestWaitForCustomAttributes_Timeout(t *testing.T) {
	withFastPolling(t, 50*time.Millisecond, 5*time.Millisecond, 10*time.Millisecond)
	stale := `{
		"arn": "arn:aws:iam::123:root", "canonicalId": "abc", "id": "123",
		"emailAddress": "e@e", "name": "test", "createDate": "2026-01-01",
		"quotaMax": 0,
	...
	server := getAccountServer(t, stale)
	defer server.Close()
	...
	if !strings.Contains(err.Error(), "did not become visible") {
		t.Errorf("error = %q, want 'did not become visible'", err.Error())
	}
	if !strings.Contains(err.Error(), "env:old") || !strings.Contains(err.Error(), "env:new") {
		t.Errorf("error = %q, want it to surface both expected and last-seen attrs", err.Error())
	}
}

func TestWaitForCustomAttributes_AccountDisappeared(t *testing.T) {
	withFastPolling(t, 50*time.Millisecond, 5*time.Millisecond, 10*time.Millisecond)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(404)
	}))
	...
	client := NewIAMClient(server.URL, "ak", "sk", false)
	err := client.WaitForCustomAttributes(context.Background(), "missing", map[string]string{"env": "prod"})
	if err == nil {
		t.Fatal("expected error for vanished account, got nil")
	}
	if !strings.Contains(err.Error(), "disappeared") {
		t.Errorf("error = %q, want it to contain 'disappeared'", err.Error())
	}
}
