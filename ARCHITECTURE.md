...
│   ├── iam.go                  # IAM client: SigV4 signing, account/user/group/policy ops
│   ├── console.go              # Console client: JWT auth, token caching
...
Each resource is a package with two files:
...
|--------|------|-------------|---------|
| `IAMClient` | SigV4 (service `iam`) | form-encoded actions | account, user, group, policy, access key resources |
| `S3Client` | SigV4 (service `s3`) | XML (JSON for bucket policy) | bucket and bucket sub-feature resources |
| `ConsoleClient` | JWT (`x-access-token` header) | JSON/REST | console account resource |
